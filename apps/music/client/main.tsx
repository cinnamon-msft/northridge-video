...
import type { MusicProduct, Page } from '../api/index.ts';
...
function Catalog() {
  const [data, setData] = useState<Page<MusicProduct> | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1);

  useEffect(() => {
    fetch('/music/api/products?page=' + page)
      .then((res) => {
        if (!res.ok) throw new Error('Failed to load catalog: ' + res.status);
        return res.json() as Promise<Page<MusicProduct>>;
      })
      .then(setData)
  ...
  if (!data) return <div className="container my-4"><p className="text-muted">Loading the music department…</p></div>;

  return (
    <main className="container my-4">
      <h1>Music</h1>
      <p className="text-muted">
        Vinyl, CDs, cassettes, turntables and speakers.{' '}
            ...
            <div className="card h-100">
              <div className="card-body d-flex flex-column">
                <h5 className="card-title">
                  <a className="nrv-detail-link" href={'/music/' + item.sku}>
                    {item.title}
                  </a>
                </h5>
...
function Detail({ sku }: { sku: string }) {
  const [item, setItem] = useState<MusicProduct | null>(null);
  const [status, setStatus] = useState<'loading' | 'ok' | 'missing'>('loading');

  useEffect(() => {
    fetch('/music/api/products/' + encodeURIComponent(sku))
      .then((res) => {
        if (res.status === 404) {
          setStatus('missing');
          return null;
        ...
        return res.json() as Promise<MusicProduct>;
      })
      .then((data) => {
        if (data) {
          setItem(data);
    ...
    return (
      <div className="container my-4">
        <div className="alert alert-warning">
          That item could not be found. <a href="/music/">Back to Music</a>.
        </div>
      </div>
    );
  ...
  return (
    <main className="container my-4">
      <p className="mb-2">
        <a href="/music/">&laquo; Back to Music</a>
      </p>
      <div className="card">
        <div className="card-body">
          ...
          <p>
            ...
            {item.artist && <span className="ml-1">{item.artist}</span>}
          </p>
...
// Route on the path: /music/<SKU> shows a detail page, otherwise the catalog.
function App() {
  const m = window.location.pathname.match(/^\/music\/(MUS-[^/]+)\/?$/);
